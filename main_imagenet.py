...
import torchvision
from engine.utils.imagenet_utils import presets, transforms, utils
...
def get_pruner(model, example_inputs, args):
    unwrapped_parameters = [model.encoder.pos_embedding, model.class_token] if "vit" in args.model else None
...
def main(args):
    ...
    )  # torchvision.models.__dict__[args.model](pretrained=args.pretrained) #torchvision.models.get_model(args.model, weights=args.weights, num_classes=num_classes)
    model.eval()
    print("=" * 16)
    print(model)
    ...
    if args.prune:
        ...
        if args.sparsity_learning:
            if args.sl_resume:
                ...
                if args.sl_lr is None:
                    args.sl_lr = args.lr
                if args.sl_lr_step_size is None:
                    args.sl_lr_step_size = args.lr_step_size
                if args.sl_lr_warmup_epochs is None:
                    args.sl_lr_warmup_epochs = args.lr_warmup_epochs
                if args.sl_epochs is None:
                    args.sl_epochs = args.epochs
                train(
                    model,
                    args.sl_epochs,
                    lr=args.sl_lr,
                    lr_step_size=args.sl_lr_step_size,
                    lr_warmup_epochs=args.sl_lr_warmup_epochs,
    ...
    train(
        ...
        args=args,
        pruner=None,
        state_dict_only=(not args.prune),
    )
...
def train(
    ...
    if args.lr_scheduler == "steplr":
        ...
        main_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs - lr_warmup_epochs, eta_min=args.lr_min)
    ...
    if lr_warmup_epochs > 0:
        if args.lr_warmup_method == "linear":
            ...
            warmup_lr_scheduler = torch.optim.lr_scheduler.ConstantLR(optimizer, factor=args.lr_warmup_decay, total_iters=lr_warmup_epochs)
        else:
