...
class _FlattenIndexMapping(object):
    ...
    def __call__(self, idxs: _HybridIndex):
        ...
        if self.reverse == True:
            ...
            for i in idxs:
                new_idxs.extend([_HybridIndex(idx=k, root_idx=i.root_idx) for k in range(i.idx * self._stride, (i.idx + 1) * self._stride)])
        return new_idxs
...
class _ConcatIndexMapping(object):
    ...
    def __call__(self, idxs: _HybridIndex):
        if self.reverse == True:
            new_idxs = [_HybridIndex(idx=i.idx - self.offset[0], root_idx=i.root_idx) for i in idxs if (i.idx >= self.offset[0] and i.idx < self.offset[1])]
        else:
...
class _GQAIndexMapping(object):
    ...
    def __call__(self, idxs: _HybridIndex):
        ...
        if self.reverse == True:
            new_idxs = [
                _HybridIndex(
                    idx=(i.idx - i.idx // (head_dim * repeat) * head_dim * (repeat - 1) - i.idx // head_dim % repeat * head_dim),
                    root_idx=None,
                )
                for i in idxs
            ]
