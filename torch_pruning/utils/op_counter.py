...
def multihead_attention_counter_hook(multihead_attention_module, input, output):
    ...
    batch_first = multihead_attention_module.batch_first if hasattr(multihead_attention_module, "batch_first") else False
    if batch_first:
...
def start_flops_count(self, **kwargs):
    ...
    def add_flops_counter_hook_function(module, ost, verbose, ignore_list):
        if type(module) in ignore_list:
            ...
            if verbose and type(module) not in (nn.Sequential, nn.ModuleList) and type(module) not in seen_types:
                print(
