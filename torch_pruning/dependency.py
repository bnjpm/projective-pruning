...
class Node(object):
    ...
    def details(self):
        ...
        for dep in self.dependencies:
            fmt += " " * 8 + "{}\n".format(dep)
        fmt += "\tenable_index_mapping={}, pruning_dim={}\n".format(self.enable_index_mapping, self.pruning_dim)
        fmt = "-" * 32 + "\n"
        return fmt
...
class Dependency(object):
    ...
    def __eq__(self, other):  # check if two dependencies are the same
        return self.source == other.source and self.trigger == other.trigger and self.handler == other.handler and self.target == other.target
...
class Group(object):
    ...
    def has_pruning_op(self, dep: Dependency, idxs: _helpers._HybridIndex):
        for _dep, _idxs in self._group:
            ...
            if _dep.target == dep.target and _dep.handler == dep.handler and _idxs == idxs:
                return True
        return False
...
class DependencyGraph(object):
    ...
    def pruning_history(
        self,
    ) -> typing.List[typing.Tuple[str, bool, typing.Union[list, tuple]]]:
    ...
    def get_pruning_group(
        ...
        if module not in self.module2node:
            raise ValueError("Module {} is not in the dependency graph.".format(module))
                ...
                for new_dep in node.dependencies:
                    if new_dep.is_triggered_by(fn):
                        ...
                        if (new_dep.target in visited_node) and group.has_pruning_op(new_dep, new_indices):
                            ...
                            processing_stack.append((new_dep, new_indices))
    ...
    def _trace(self, model, example_inputs, forward_fn, output_transform):
        ...
        hooks = [m.register_forward_hook(_record_grad_fn) for m in model.modules() if (isinstance(m, registered_types) and m not in self.IGNORED_LAYERS_IN_TRACING)]
        ...
        for o in utils.flatten_as_list(out):
            self._trace_computational_graph(module2node, o.grad_fn, gradfn2module, reused, visited=visited)
        ...
        def create_node_if_not_exists(grad_fn):
            ...
            if module is None:  # a new module
                if not hasattr(grad_fn, "name"):
                    ...
                    self._op_id += 1
                    if self.verbose:
                        warnings.warn("[Warning] Unknown operation {} encountered, which will be handled as an element-wise op".format(str(grad_fn)))
            ...
            if module not in module2node:
                ...
                if type(module) in self.CUSTOMIZED_PRUNERS:  # mark it as a customized layer
        ...
        while len(processing_stack) > 0:
            ...
            if hasattr(grad_fn, "next_functions"):
                for f in grad_fn.next_functions:
                    if f[0] is not None:
                        if hasattr(f[0], "name") and "accumulategrad" in f[0].name().lower():  # a leaf variable.
    ...
    def _update_slice_index_mapping(self, slice_node: Node):
        ...
        start, step, end, dim = (
            slice_node.module.start,
            slice_node.module.step,
            slice_node.module.end,
            slice_node.module.dim,
        )
        for node in slice_node.inputs:
            for dep in slice_node.dependencies:
                if dep.target == node:
                    dep.index_mapping[0] = _helpers._SliceIndexMapping(dim=dim, start=start, end=end, step=step, reverse=True)
            for dep in node.dependencies:
                if dep.target == slice_node:
                    dep.index_mapping[0] = _helpers._SliceIndexMapping(dim=dim, start=start, end=end, step=step, reverse=False)
            ...
            for in_node in fc_node.inputs:
                for dep in fc_node.dependencies:
                    if dep.target == in_node:
                        dep.index_mapping[0] = _helpers._FlattenIndexMapping(stride=stride, reverse=True)
                for dep in in_node.dependencies:
                    if dep.target == fc_node:
                        dep.index_mapping[0] = _helpers._FlattenIndexMapping(stride=stride, reverse=False)
        ...
        if out_channels > in_channels:
            for in_node in reshape_node.inputs:
                for dep in reshape_node.dependencies:
                    if dep.target == in_node:
                        dep.index_mapping[0] = _helpers._FlattenIndexMapping(stride=out_channels // in_channels, reverse=True)

                for dep in in_node.dependencies:
                    if dep.target == reshape_node:
                        dep.index_mapping[0] = _helpers._FlattenIndexMapping(stride=out_channels // in_channels, reverse=False)
        else:  # 1D -> 2D
            for out_node in reshape_node.outputs:
                for dep in reshape_node.dependencies:
                    if dep.target == out_node:
                        dep.index_mapping[0] = _helpers._FlattenIndexMapping(stride=in_channels // out_channels, reverse=True)

                for dep in out_node.dependencies:
                    if dep.target == reshape_node:
                        dep.index_mapping[0] = _helpers._FlattenIndexMapping(stride=in_channels // out_channels, reverse=False)
    ...
    def _update_concat_index_mapping(self, cat_node: Node):
        ...
        for i, in_node in enumerate(cat_node.inputs):
            for dep in cat_node.dependencies:
                if any((dep is d) for d in addressed_dep):
                    continue
                if dep.target == in_node:
                    if cat_node.enable_index_mapping:
                        dep.index_mapping[1] = _helpers._ConcatIndexMapping(offset=offsets[i : i + 2], reverse=True)
        ...
        for i, in_node in enumerate(cat_node.inputs):
            for dep in in_node.dependencies:
                ...
                if dep.target == cat_node:
                    if cat_node.enable_index_mapping:
                        dep.index_mapping[1] = _helpers._ConcatIndexMapping(offset=offsets[i : i + 2], reverse=False)
                        addressed_dep.append(dep)
                        break
            ...
            for dep in split_node.dependencies:
                ...
                if dep.target == out_node:
                    if split_node.enable_index_mapping:
                        dep.index_mapping[0] = _helpers._SplitIndexMapping(offset=offsets[i : i + 2], reverse=False)
                        addressed_dep.append(dep)
                        break
            ...
            for dep in out_node.dependencies:
                if dep.target == split_node:
                    if any((dep is d) for d in addressed_dep):
                        continue
                    if split_node.enable_index_mapping:
                        dep.index_mapping[0] = _helpers._SplitIndexMapping(offset=offsets[i : i + 2], reverse=True)
                        addressed_dep.append(dep)
                        break
