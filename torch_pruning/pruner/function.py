...
class BasePruningFunc(ABC):
    ...
    def check(self, layer, idxs, to_output):
        if self.TARGET_MODULES is not None:
            assert isinstance(layer, self.TARGET_MODULES), "Mismatched pruner {} and module {}".format(self.__str__, layer)
        if to_output:
        ...
        if prunable_channels is not None:
            assert all(idx < prunable_channels and idx >= 0 for idx in idxs), "All pruning indices should fall into [{}, {})".format(0, prunable_channels)
    ...
    def _prune_parameter_and_grad(self, weight, keep_idxs, pruning_dim):
        pruned_weight = torch.nn.Parameter(
            torch.index_select(
                weight,
                pruning_dim,
                torch.LongTensor(keep_idxs).to(weight.device).contiguous(),
            )
        )
...
class LSTMPruner(BasePruningFunc):
    ...
    def prune_out_channels(self, layer: nn.LSTM, idxs: list) -> nn.Module:
        ...
        for pf in postfix:
            setattr(
                layer,
                "weight_hh_l0" + pf,
                self._prune_parameter_and_grad(getattr(layer, "weight_hh_l0" + pf), keep_idxs, 0),
            )
            if layer.bias:
                setattr(
                    layer,
                    "bias_hh_l0" + pf,
                    self._prune_parameter_and_grad(getattr(layer, "bias_hh_l0" + pf), keep_idxs, 0),
                )
            setattr(
                layer,
                "weight_hh_l0" + pf,
            ...
            setattr(
                layer,
                "weight_ih_l0" + pf,
                self._prune_parameter_and_grad(getattr(layer, "weight_ih_l0" + pf), expanded_keep_idxs, 1),
            )
            if layer.bias:
                setattr(
                    layer,
                    "bias_ih_l0" + pf,
                    self._prune_parameter_and_grad(getattr(layer, "bias_ih_l0" + pf), keep_idxs, 0),
                )
    ...
    def prune_in_channels(self, layer: nn.LSTM, idxs: list):
        ...
        setattr(
            layer,
            "weight_ih_l0",
            self._prune_parameter_and_grad(getattr(layer, "weight_ih_l0"), keep_idxs, 1),
        )
        if layer.bidirectional:
            setattr(
                layer,
                "weight_ih_l0_reverse",
                self._prune_parameter_and_grad(getattr(layer, "weight_ih_l0_reverse"), keep_idxs, 1),
            )
