...
class MetaPruner:
    ...
    def __init__(
        ...
        self.per_step_pruning_ratio = self.iterative_pruning_ratio_scheduler(self.pruning_ratio, self.iterative_steps)
        self.per_step_head_pruning_ratio = self.iterative_pruning_ratio_scheduler(self.head_pruning_ratio, self.iterative_steps)
                ...
                for m in scope:
                    for submodule in m.modules():
                        ...
                        if isinstance(submodule, prunable_types):
                            if isinstance(submodule, nn.Module):
                                if not self.global_pruning:
                                    self._layer_to_scope[submodule] = (
                                        scope_name + f"_{local_pruning_scope_postfix}",
                                        scope,
                                    )
                            ...
                            self.pruning_ratio_dict[submodule] = self.iterative_pruning_ratio_scheduler(ratio, self.iterative_steps)
        ...
        if head_pruning_ratio_dict is not None:
            for module in head_pruning_ratio_dict:
                ...
                for submodule in module.modules():
                    prunable_types = tuple([ops.type2class(prunable_type) for prunable_type in self.DG.REGISTERED_PRUNERS.keys()])
                    if isinstance(submodule, prunable_types):
                        self.head_pruning_ratio_dict[submodule] = self.iterative_pruning_ratio_scheduler(ratio, self.iterative_steps)
        ...
        for dep, _ in group:
            ...
            if self.DG.is_out_channel_pruning_fn(pruning_fn):
                ...
                if layer_out_ch is None:
                    continue
                if layer_out_ch < self.layer_init_out_ch[module] * (1 - self.max_pruning_ratio) or layer_out_ch == 1:
                    return False
                ...
                if layer_in_ch is None:
                    continue
                if layer_in_ch < self.layer_init_in_ch[module] * (1 - self.max_pruning_ratio) or layer_in_ch == 1:
                    return False
        return True
    ...
    def _prune(self) -> typing.Generator:
        ...
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types):
            if self._check_pruning_ratio(group):
                ...
                for dep, _ in group:
                    for module, pruning_fn in zip(
                        [dep.source.module, dep.target.module],
                        [dep.trigger, dep.handler],
                    ):
                ...
                if self.isomorphic:
                    ...
                    for (
                        dep,
                        _,
                    ) in group:  # if isomorphic, the source and target modules should have the same **layer type** and **pruning function**
                        source = "%s_%s" % (
                            type(dep.source.module),
                            "out" if self.DG.is_out_channel_pruning_fn(dep.handler) else "in",
                        )
                        target = "%s_%s" % (
                            type(dep.target.module),
                            "out" if self.DG.is_out_channel_pruning_fn(dep.handler) else "in",
                        )
        ...
        if len(ranking_scope[ATTN_HEAD_SCOPE]) > 0 and self.global_pruning:
            concat_head_imp = torch.cat(
                [local_imp[-1] for local_imp in ranking_scope[ATTN_HEAD_SCOPE].values()],
                dim=0,
            )
            target_head_pruning_ratio = self.per_step_head_pruning_ratio[self.current_step]
            n_heads_removed = len(concat_head_imp) - int(self.initial_total_heads * (1 - target_head_pruning_ratio))
        ...
        for scope_id, scope_name in enumerate(width_pruning_scope_names):
            ...
            if len(records) > 0:
                ...
                n_pruned = len(concat_imp) - int(self._scope_initial_channels[scope_name] * (1 - target_pruning_ratio))
