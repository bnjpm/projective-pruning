...
# For pruning
parser.add_argument("--last-epochs", default=100, type=int)
    ...
    while current_speed_up < speed_up:
        if args.method == "obdc":
            ...
            for k, (imgs, lbls) in enumerate(train_loader):
                ...
                output = model(imgs)
                sampled_y = torch.multinomial(torch.nn.functional.softmax(output.cpu().data, dim=1), 1).squeeze().to(args.device)
...
def train_model(
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=lr_decay_gamma)
    model.to(device)
    best_acc = -1
...
def get_pruner(model, example_inputs):
    ...
    if args.method == "random":
        ...
        imp = tp.importance.ProjImportance()
        pruner_entry = partial(tp.pruner.MagnitudePruner, global_pruning=args.global_pruning)
    elif args.method == "proj_sl":
        args.sparsity_learning = True
        imp = tp.importance.ProjImportance(normalizer="max")
...
def main():
    ...
    if args.mode == "prune":
        ...
        logger_name = "{}-{}-{}-{}-{}".format(args.dataset, prefix, args.method, args.speed_up, args.model)
    ...
    num_classes, train_dst, val_dst, input_size = registry.get_dataset(args.dataset, data_root=args.dataroot)
    ...
    example_inputs = torch.Tensor(train_dst[0][0]).unsqueeze(0).to(args.device)
